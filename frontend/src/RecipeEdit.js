...
export function RecipeEdit(props) {
  ...
  function fetchRecipeBySlug() {
    return fetch(`https://recept-app.fly.dev/api/recipes/${params.recipeSlug}`)
      .then((res) => res.json())
      .then((recipe) => {
        setRecipe(recipe);
  ...
  return (
    <div className="card p-3 mt-5 pt-5">
      ...
      <form
        onSubmit={async (e) => {
          ...
          setPending(true);
          await fetch(`https://recept-app.fly.dev/api/recipes/${recipe.id}`, {
            method: "PUT",
            body: formData,
          });
